...
医療・健康関連の情報検索・診断支援ツール（Shell スクリプト中心）。複数 AI エージェント（トリアージ・専門医・EBM レビュー等）を組み合わせて医療情報レポートを生成するシステム。
...
3. **`agents/`** — AIエージェント定義（00〜05）
4. **`session.json`** — 進捗状態（存在する場合）。途中停止時は `/continue` で再開

> 編集前に、ユーザー発話のキーワードを `agents/` ディレクトリと照合してから対象ファイルを特定する（推測で着手しない）。

---

## 2. 単独完結注記・プロジェクト構成

本リポ CLAUDE.md は Web版 Claude Code でも単独で機能するよう設計されています。

**主要ディレクトリ構成:**
- `agents/` — AIエージェント定義（00_safety_net 〜 05_patient_translator）
- `medical/` — 医療ロジック定義（triage, differential_dx, ebm_evaluation 等）
- `schemas/` — JSON Schema（intake, report, triage）
- `reports/` — 生成レポート出力先
- `sessions/` — セッションデータ
- `scripts/publish_report.sh` — レポート公開スクリプト
- `.claude/commands/consult.md` — カスタムコマンド定義
...
追加参照ファイル（存在する場合）：
- `.claude/cross-repo.md` — 関連リポ連携ルール
- `.claude/quality-rules.md` — 品質ルール
- `.env.example` — 環境変数テンプレート（実際の `.env` は `.gitignore` 対象）
...
3. **報告前に URL 存在確認**: `gh api repos/KazuyaMurayama/Doctor/contents/PATH?ref=BRANCH` でステータス200確認。
...
## 9. ドキュメント命名・日付ルール（v2.0 / 2026-06-03 改訂）

### ファイル名
- 基本形 `<TOPIC>_YYYYMMDD.md`（**サフィックス・ハイフンなし**）。例: `MEDICAL_REPORT_20260603.md`
- 同日中の追加更新は `-v2`、`-v3`（例: `MEDICAL_REPORT_20260603-v2.md`）。
- 日付が変わったら v サフィックスをリセット（例: 翌日1回目 `MEDICAL_REPORT_20260604.md`）。
...
| トリガー | スキル |
|---|---|
| アイデア出し・エージェント設計の検討 | `.claude/skills/sp-brainstorming/SKILL.md` |
| 計画立案 | `.claude/skills/sp-writing-plans/SKILL.md` |
| 計画に沿った実行 | `.claude/skills/sp-executing-plans/SKILL.md` |
| 図表生成（フロー・シーケンス） | `.claude/skills/mermaid-agents365/SKILL.md` |
